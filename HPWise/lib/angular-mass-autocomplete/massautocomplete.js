  ...
  return {
    ...
    controller: ["$scope", function ($scope) {
      ...
      var user_options = {
        ...
        debounce_suggest: _user_options.debounce_suggest || 500,
      ...
      function _position_autocomplete() {
        var rect = current_element[0].getBoundingClientRect(),
          ...
          if ( /Windows Phone 8|Windows Phone 8.1/i.test(navigator.userAgent) ) {
            container.style.top = rect.top + rect.height + 'px';
          }else if((/Windows NT/i.test(navigator.userAgent)) && (/Tablet/i.test(navigator.userAgent))){
            $scope.show_autocomplete = false;  
            setTimeout(function(){
              $scope.show_autocomplete = true;
              var rectangle = current_element[0].getBoundingClientRect();
              container.style.top = $('#inputSearchCust').offset().top + rectangle.height - $(window).scrollTop() + 'px';
             },1000);
          }else{
